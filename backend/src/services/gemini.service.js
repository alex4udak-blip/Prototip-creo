...
2. Когда указан [VARIANTS:X] — генерируй РОВНО X ОТДЕЛЬНЫХ изображений!
   ❌ НЕ делай коллаж! НЕ склеивай варианты в одну картинку!
   ✅ Каждый вариант = ОТДЕЛЬНЫЙ файл изображения
   Не меньше, не больше. Каждое изображение должно быть УНИКАЛЬНЫМ!
...
⚠️ КРИТИЧЕСКИ ВАЖНО:
1. Каждый вариант = ОТДЕЛЬНЫЙ ФАЙЛ изображения! НЕ делай коллаж!
2. СОХРАНИ персонажа из референса (рисуй ТОГО ЖЕ человека!)
3. СОХРАНИ механику (коробки/слоты/колесо — как в референсе)
4. СОХРАНИ цвета и стиль оформления
5. МЕНЯЙ только: заголовки, текст бонуса, позицию CTA

Выведи ${variants} ОТДЕЛЬНЫХ изображений (не коллаж!), потом краткое описание.`;
...
Применяй запрошенные изменения к КАЖДОМУ изображению.\n\n`;
  ...
  // Проверяем: AI задал вопросы и ждёт ответа?
  // Если в тексте есть вопросы и нет картинок — это SMART режим, ждём ответа пользователя
  const hasQuestions = result.text && result.text.includes('?');
  const isWaitingForAnswer = hasQuestions && result.images.length === 0 && !settings.isFollowUp;

  if (isWaitingForAnswer) {
    log.info('AI asked questions, waiting for user answer - skipping image generation', {
      chatId,
      textPreview: result.text.substring(0, 100)
    });
    return result;  // Возвращаем ответ с вопросами, НЕ генерируем картинки
  }
  ...
  log.info('Checking if need more images', {
    ...
    isFollowUp: settings.isFollowUp
  });
  ...
  while (result.images.length < targetVariants && retryCount < maxRetries) {
    ...
    try {
      const moreMessage = `Сгенерируй ещё ${remaining} ОТДЕЛЬНЫХ изображений (НЕ коллаж!). Каждое — отдельный файл. СОХРАНИ персонажа и механику, МЕНЯЙ текст/CTA.`;
