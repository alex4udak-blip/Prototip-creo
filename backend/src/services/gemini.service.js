...
## 🚨🚨🚨 РЕФЕРЕНСЫ — ЭТО ПРИМЕРЫ СТИЛЯ! 🚨🚨🚨

⚠️ КРИТИЧЕСКИ ВАЖНО: Когда пользователь присылает картинки — это РЕФЕРЕНСЫ!
❌ НЕ редактируй эти картинки!
❌ НЕ добавляй фильтры/шум/эффекты к ним!
❌ НЕ возвращай эти же картинки с изменениями!

✅ СОЗДАВАЙ НОВЫЕ ОРИГИНАЛЬНЫЕ БАННЕРЫ, вдохновлённые стилем референса!
✅ Проанализируй референс: цвета, композиция, стиль текста, механика
✅ Создай СОВЕРШЕННО НОВЫЕ изображения с аналогичным визуальным стилем
✅ Каждый вариант должен быть УНИКАЛЬНЫМ — разные композиции, персонажи, элементы
...
## 📸 РЕФЕРЕНСЫ (для вдохновения, НЕ для редактирования!)

Когда пользователь прислал картинки как примеры стиля:
1. Опиши что видишь: "Вижу казино-креатив с механикой выбора, яркие неоновые цвета..."
2. Спроси уточнения (в режиме [SMART])
3. При генерации — создавай НОВЫЕ баннеры В ПОХОЖЕМ СТИЛЕ:
   - Сохраняй цветовую гамму и настроение
   - Используй похожую композицию
   - Копируй стиль текста и CTA
   - НО: новые персонажи, новые элементы, новый контент!
...
export async function sendMessageStream(chatId, text, images = [], settings = {}, onProgress) {
  ...
  // Follow-up: пользователь ответил на вопросы AI — ГЕНЕРИРОВАТЬ КАРТИНКИ!
    ...
    fullText = fullText + `\n\n---
[GENERATE_NOW] Вся информация получена. СГЕНЕРИРУЙ ${variants} НОВЫХ ОРИГИНАЛЬНЫХ баннеров ПРЯМО СЕЙЧАС.

⚠️ ВАЖНО:
- НЕ редактируй референсы! НЕ добавляй к ним фильтры/шум!
- Создай ${variants} СОВЕРШЕННО НОВЫХ изображений, вдохновлённых стилем референса
- Каждый баннер должен быть УНИКАЛЬНЫМ: разные композиции, элементы, персонажи
- Сохрани только СТИЛЬ: цвета, настроение, тип CTA

Сначала выведи ${variants} изображений, потом краткое описание (1-2 предложения).`;
...
Применяй запрошенные изменения к КАЖДОМУ изображению.\n\n`;
  ...
  if (settings.isFollowUp && result.images.length === 0 && result.text && !settings._retryAttempt) {
    ...
    const variants = settings.variants || 3;
    const retryMessage = `Теперь СГЕНЕРИРУЙ ${variants} НОВЫХ ОРИГИНАЛЬНЫХ баннеров.
НЕ описывай — СОЗДАЙ реальные картинки прямо сейчас!
НЕ редактируй референсы — создай НОВЫЕ изображения в похожем стиле!
Каждый баннер должен быть УНИКАЛЬНЫМ.`;
