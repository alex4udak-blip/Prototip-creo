...
export async function sendMessage(chatId, text, images = [], settings = {}) {
  ...
  // Если есть референсы
  if (images.length > 0) {
    // В SMART режиме — сначала анализ и вопросы, генерация только после подтверждения
    if (settings.mode !== 'fast' && !hasExplicitGenerateRequest) {
      fullText += '\n\n📸 Вижу референс(ы). Проанализируй их и задай уточняющие вопросы перед генерацией.';
    } else {
      // В FAST режиме или при явном запросе — генерируй сразу
