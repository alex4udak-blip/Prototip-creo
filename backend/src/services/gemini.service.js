...
export async function sendMessage(chatId, text, images = [], settings = {}) {
  ...
  if (settings.mode === 'fast') {
    ...
    // Режим умный - СНАЧАЛА задай вопросы (если нет явного запроса на генерацию)
    const hasExplicitGenerateRequest = /генер|созда|сделай|делай|нарисуй|давай/i.test(text || '');
    if (!hasExplicitGenerateRequest) {
  ...
  // Если есть референсы и явный запрос на генерацию - добавляем инструкцию
  if (images.length > 0) {
    const wantsGeneration = /генер|созда|сделай|делай|нарисуй|давай|похож|стиль|так же/i.test(text || '');
    if (wantsGeneration || settings.mode === 'fast') {
      fullText += '\n[GENERATE_IMAGES: обязательно сгенерируй изображения в стиле референса]';
    }
  }
