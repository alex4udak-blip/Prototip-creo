...
### 🧠 Режим "Умный" [SMART] (по умолчанию):
⚠️ КРИТИЧЕСКИ ВАЖНО: Когда видишь тег [SMART] в начале сообщения:
- НЕ ГЕНЕРИРУЙ изображения сразу!
- СНАЧАЛА задай 2-4 уточняющих вопроса
- Дождись ответа пользователя перед генерацией

Задай 2-4 УМНЫХ вопроса как опытный медиабайер:

1. **Что рекламируем?** — Название приложения/игры, жанр, или "лендо-крео" без бренда?

2. **Оффер** — Какой бонус? Сумма, условия (депозит/no deposit)?

3. **ГЕО** — Страна/регион? Язык текста на креативе?

4. **Ограничения** — Можно персонажей или store-friendly? Есть брендбук?

### 💡 После ответов пользователя:
Предложи 2-4 концепции НА ВЫБОР (текстом, без картинок):
- **Концепция 1:** краткое описание
- **Концепция 2:** краткое описание
- **Концепция 3:** краткое описание

Спроси: "Какой вариант генерировать? Или все сразу?"

### 🎨 Генерация изображений:
Генерируй изображения когда:
- Видишь тег [GENERATE_IMAGES] — ОБЯЗАТЕЛЬНО генерируй изображения!
- Пользователь выбрал концепцию
- Или явно попросил "генерируй", "делай", "создай"
- Или режим [FAST]

⚠️ ВАЖНО: Когда есть тег [GENERATE_IMAGES] — ты ДОЛЖЕН сгенерировать реальные изображения, а не просто описать их!

### ⚡ Режим "Быстрый" [FAST]:
Когда в запросе есть [FAST] или слова "быстро", "сразу", "без вопросов", "давай":
- Генерируй изображения СРАЗУ без вопросов
- Используй информацию из референсов + разумные defaults
- Создай 2-3 варианта с разными подходами

### 🎨 При генерации изображений:
1. Создай КАЧЕСТВЕННЫЕ изображения
...
export async function sendMessage(chatId, text, images = [], settings = {}) {
  ...
  // Определяем есть ли явный запрос на генерацию
  const hasExplicitGenerateRequest = /генер|созда|сделай|делай|нарисуй|давай|быстро|сразу/i.test(text || '');

  // isFollowUp = это ответ на вопросы AI (не первое сообщение в чате)
  const isFollowUp = settings.isFollowUp === true;

  // Режим быстрый - генерируй сразу
  if (settings.mode === 'fast') {
    fullText = '[FAST] ' + fullText;
  } else if (isFollowUp) {
    // Это ответ на вопросы AI — генерируем изображения!
    fullText = '[GENERATE_IMAGES] ' + fullText;
  } else if (!hasExplicitGenerateRequest) {
    // Режим умный - СНАЧАЛА задай вопросы (только если нет явного запроса)
    fullText = '[SMART] ' + fullText;
  }
  ...
  // Если есть референсы
  if (images.length > 0) {
    // В SMART режиме (первое сообщение) — сначала анализ и вопросы
    if (settings.mode !== 'fast' && !hasExplicitGenerateRequest && !isFollowUp) {
      fullText += '\n\n📸 Вижу референс(ы). Проанализируй их и задай уточняющие вопросы перед генерацией.';
    } else {
      // В FAST режиме, при явном запросе или follow-up — генерируй сразу
      fullText += '\n\n⚠️ ОБЯЗАТЕЛЬНО: Проанализируй референс и СГЕНЕРИРУЙ новые изображения в похожем стиле. Не просто описывай — СОЗДАЙ реальные изображения!';
    }
  }

  // Если это follow-up без референсов — тоже добавляем инструкцию генерировать
  if (isFollowUp && images.length === 0) {
    fullText += '\n\n⚠️ Пользователь ответил на твои вопросы. Теперь ОБЯЗАТЕЛЬНО СГЕНЕРИРУЙ изображения на основе полученной информации!';
  }
...
export async function sendMessageStream(chatId, text, images = [], settings = {}, onProgress) {
  ...
  // isFollowUp = это ответ на вопросы AI (не первое сообщение в чате)
  // Если пользователь отвечает на вопросы — нужно генерировать изображения
  const isFollowUp = settings.isFollowUp === true;
